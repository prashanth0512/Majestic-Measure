document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hero) {
        ...
        const startTimeline = () => {
            ...
            setAnimatePath(sketchPaths.collar, 300, 100);
            setAnimatePath(sketchPaths.lapelL, 350, 200);
            setAnimatePath(sketchPaths.lapelR, 350, 200);
            setAnimatePath(sketchPaths.sleeveL, 400, 350);
            setAnimatePath(sketchPaths.sleeveR, 400, 350);
            setAnimatePath(sketchPaths.pocket, 250, 500);
            setAnimatePath(sketchPaths.buttons, 250, 600);
            const tBlueprint = setTimeout(() => {
                hero.classList.add('state-blueprint');
                if (blueprintMarks) {
                    blueprintMarks.style.transition = 'opacity 0.5s ease-in-out';
            ...
            const tThreads = setTimeout(() => {
                if (goldenThreadsGroup) {
                    goldenThreadsGroup.style.transition = 'opacity 0.4s ease';
                    goldenThreadsGroup.style.opacity = '1';
                }
                threadPaths.forEach(path => {
                    path.style.transition = 'stroke-dashoffset 0.8s cubic-bezier(0.25, 1, 0.5, 1)';
                    path.style.strokeDashoffset = '0';
                });
                if (blueprintMarks) {
                    blueprintMarks.style.transition = 'opacity 0.6s cubic-bezier(0.25, 1, 0.5, 1)';
            ...
            const tZoom = setTimeout(() => {
                ...
                const sketchOverlay = document.querySelector('.layer-sketch');
                if (sketchOverlay) {
                    sketchOverlay.style.transition = 'opacity 0.8s ease';
